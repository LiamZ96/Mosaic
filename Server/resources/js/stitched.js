...
$(window).bind("load", function(){
	var addImages = function(){
		...
		let address = $(location).attr('href'),
			...
			r = $.Deferred(),
			imageLocation = "/resources/uploads/" + last_part + "/maps/";
		for(let i = 0; i < ImageDiv.dataset.numfiles-1;i++){
			$("#ImageDiv").append( "<img src='"+ imageLocation + "stitched_image"+ i +".jpg" +"' id=image"+ i +">");
		}
		return r;
	};
	var shrinkImages = function(){
		for(let j = 0; j < ImageDiv.dataset.numfiles-1;j++){
